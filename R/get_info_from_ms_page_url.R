get_info_from_ms_page_url <- function(ms_page_url){
  ...
  ret_info$SeneddEmail <-
    ret_info$Emails[ret_info$Emails %>% str_detect("senedd\\.wales$")]
  ...
  ret_info$SeneddPhone <-
    ret_info$Phones[ret_info$Phones %>% str_detect("^0300")]
  ...
  ret_info$TwitterURL <-
    ret_info$SocialURLs[ret_info$SocialURLs %>% str_detect("^https?://(www\\.)?twitter\\.com")]

  ret_info$FacebookURL <-
    ret_info$SocialURLs[ret_info$SocialURLs %>% str_detect("^https?://(www\\.)?facebook\\.com")]
